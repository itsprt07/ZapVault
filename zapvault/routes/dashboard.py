...
def dashboard():
    ...
    last_upload_str = last_upload.astimezone(IST).strftime('%Y-%m-%d %I:%M:%S %p') if last_upload else "No uploads yet"
    total_files = len(files)
    total_downloads = sum(f.download_count for f in files)
    expired_files = sum(1 for f in files if f.expired or datetime.utcnow() > f.expires_at)
    ...
    rows = ""
    for f in files:
        ist_uploaded = f.timestamp.replace(tzinfo=None).astimezone(IST)
        ist_expiry = f.expires_at.replace(tzinfo=None).astimezone(IST)
